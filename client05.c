...
struct __attribute__((packed)) mr_info
{
...
main(int argc, char **argv)
{
	...
	struct mr_info client_mr_info = {(uintptr_t)buffer, mr->rkey};
	cm_params.private_data = &client_mr_info;
	cm_params.private_data_len = sizeof(client_mr_info);
	cm_params.initiator_depth = 1;
	printf("Client private data size: %zu\n", sizeof(client_mr_info));
	...
	struct mr_info *server_mr = (struct mr_info *)event->param.conn.private_data; // Get the server's MR info
	rdma_ack_cm_event(event);
	...
	wr.wr_id = 1;
	wr.opcode = IBV_WR_RDMA_WRITE_WITH_IMM; // Use RDMA Write with Immediate
	wr.imm_data = htonl(0xDEADBEEF);        // Immediate data
	...
	// Wait for the server's RDMA Write
	printf("Busy waiting for RDMA Write from server...\n");
	uint32_t *marker = (uint32_t *)(buffer + BUFFER_SIZE - 2 * 1024 * 1024 - sizeof(uint32_t));
	while (ntohl(*marker) != 0xDEADBEEF) // Busy-wait for the marker to be set
	{
		// Busy-waiting loop
	}
